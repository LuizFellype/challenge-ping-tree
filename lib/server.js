...
router.set('/api/targets', {
  GET: (req, res) => {
    redis.keys(targetPrefix + '*', (err, keys) => {
  ...
  POST: (req, res) => {
    ...
    req.on('end', () => {
      ...
      redis.set(targetPrefix + target.id, jsonString)
    })
...
router.set('/api/target/:id', (req, res, opts) => {
  redis.get(targetPrefix + opts.params.id, (err, obj) => {
    if (err) {
      ...
      redis.keys(targetPrefix + '*', (err, keys) => {
        ...
        if (keys) {
          async.map(keys, (key, cb) => {
            ...
            results.forEach(r => {
              if (r.maxAcceptsPerDay > 0 &&
                r.accept.geoState.includes(visitor.geoState) &&
                r.accept.hour.includes(new Date(visitor.timestamp).getUTCHours()) &&
                +r.value > tempVal
              ) {
                tempVal = +r.value
                decision = r.url
            ...
            if (target) {
              ...
              redis.set(targetPrefix + target.id, JSON.stringify(target))
            }
